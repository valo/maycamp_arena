...
class RatingCalculationTest < ActiveSupport::TestCase
  def test_calculating_after_first_contest
    ...
    results = []
    results << create(:contest_result, :contest => contest, :points => rand * 300) while results.length < 10
  ...
  def test_calculating_the_rating_of_2_coders
    ...
    results = []
    results << create(:contest_result, :contest => contest, :points => 900)
    results << create(:contest_result, :contest => contest, :points => 1300)
  ...
  def test_calculating_the_rating_of_9_tied_coders
    ...
    results = []
    results << create(:contest_result, :contest => contest, :points => 1300) while results.length < 9
  ...
  def test_calculating_the_rating_of_2_coders
    ...
    results = []
    results << create(:contest_result, :user => user1, :contest => contest, :points => 150)
    results << create(:contest_result, :user => user2, :contest => contest, :points => 300)
  ...
  #
  #   users = []
  #   users << create(:user) while users.length < 50
  #
  ...
  #       results << create(:contest_result, :user => users[index], :contest => contest, :points => norm.rng * 300)
  #     end
  #
